...
const UpdateSupplier = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://159.223.57.121:8090/supplier/find-by-id/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then((res) => {
        setNamaSupplier(res.data.data.namaSupplier);
    ...
    <>
      ...
      <main className="flex justify-center">
        <div className="w-[50%] my-10 md:my-[80px]">
          <div>
            <h1 className="font-medium text-[32px] md:text-[48px] text-center">Update Supplier</h1>
          </div>
